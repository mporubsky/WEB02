/* =========================================================================
   BABYLAND — Kontaktný dopytový formulár (kontaktná stránka)
    ...
    window.MH.sendLead(data, { subject: "Nový dopyt z kontaktného formulára – " + (window.MH.config.business.shortName || "") })
      .then(function (res) {
        ...
        if (res && res.fallback === "mailto") {
          ...
          // Lehotu sľubujeme len vtedy, ak je reálne vyplnená v config.js.
          var rt = window.MH.config.business.responseTime;
          showAlert("Ďakujeme! Vašu správu sme prijali. " +
            (rt ? "Ozveme sa vám do " + rt + "." : "Ozveme sa vám čo najskôr."), "ok");
        }
      })
      .catch(function () {
