/* =========================================================================
   BABYLAND — Interaktívny dopytový sprievodca (3-krokový wizard)
   Krok 1: forma opatery → Krok 2: vek dieťaťa → Krok 3: mestská časť + telefón
...
(function () {
  ...
  var state = { sluzba: "", vek: "", city: "", phone: "" };
  var current = 0;
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (!state.sluzba || !state.vek) {
        showMsg("Prosím, dokončite kroky 1 a 2.", true);
        return;
      }
      if (!state.city.trim() || !state.phone.trim()) {
        showMsg("Zadajte prosím mestskú časť aj telefónne číslo.", true);
        return;
      }
      showMsg("", false);
      ...
      window.MH.sendLead({
        ...
        forma_opatery: state.sluzba,
        vek_dietata: state.vek,
        mesto: state.city,
        telefon: state.phone
